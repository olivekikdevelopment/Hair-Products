...
class ThemesManager<in KeyType>(private val themesRepository: IThemesRepository,
                                ...
                                private val canAccessPaidThemes: Boolean) : IThemesManager<KeyType> {
    companion object {
    ...
    constructor(themesRepository: IThemesRepository,
                themesAdapter: IThemesAdapter<KeyType, UUID>,
                ...
                mapMethod: (ConvoId) -> KeyType,
                productPaymentManager: IProductPaymentManager,
                canAccessPaidThemes: Boolean)
    ...
    private inner class ThemePaymentsHelper {
        ...
        fun purchaseTheme(themeId: UUID) {
            ...
            productPaymentManager.purchase(themeId)
        }
        ...
        fun retryPurchaseTheme(themeId: UUID) {
            ...
            themeTransactionStatus(themeId).first().subscribe { transactionStatus ->
                when (transactionStatus) {
                    ...
                    else -> productPaymentManager.retryFailedPurchase(themeId)
                }
            }
        }

        fun cancelTransaction(themeId: UUID) {
            productPaymentManager.cancelFailedPurchase(themeId)
                    .onErrorComplete()
                    .subscribe {
        ...
        fun themeTransactionStatus(themeId: UUID) =
                if (themeId == ITheme.DEFAULT_THEME_ID) Observable.just(ThemeTransactionStatus.NO_TRANSACTION)
                else themeTransactionStatusMap.get(themeId)
        ...
        private fun adaptProductTransaction(themeId: UUID) {
            ...
            doConvertProductToThemeTransaction(themeId)
                    ...
                    .filter { it.second == ThemeTransactionStatus.PENDING_REFRESH_THEME }
                    .subscribe({ refreshTheme(themeId) },
