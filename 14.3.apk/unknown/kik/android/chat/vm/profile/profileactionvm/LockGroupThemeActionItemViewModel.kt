...
class LockGroupThemeActionItemViewModel(private val jid: BareJid) : AbstractToggleItemViewModel() {
    ...
    private var readWriteLock: ReadWriteLock = ReentrantReadWriteLock()
    ...
    override fun attach(coreComponent: CoreComponent, navigator: INavigator) {
        ...
        lifecycleSubscription.add(conversation.statusThemeMessageReceivedObservable()
                .filter { message -> getIsToggleDialogVisible() && shouldShowDialog(message) }
                .subscribe { message ->
    ...
    override fun tapped() {
        ...
        val dialogViewModel = DialogViewModel.confirm(
                ...
                data.cancelTitle,
                Runnable { setIsToggleDialogVisible(false) },
                true,
                Runnable { setIsToggleDialogVisible(false) })
        navigator.showDialog(dialogViewModel)
        setIsToggleDialogVisible(true)
    }
    ...
    private fun getIsToggleDialogVisible(): Boolean {
        var value = false
        readWriteLock.readLock().withLock {
            value = isToggleDialogVisible
        }
        return value
    }

    private fun setIsToggleDialogVisible(setVal: Boolean) {
        readWriteLock.writeLock().withLock {
            isToggleDialogVisible = setVal
        }
    }
